 ...
 * the real endpoint into `<form action={...}>` — without JS the browser
 * POSTs there natively (the transport handles no-JS form posts). With JS, a
 * single document-level submit listener intercepts matching forms and calls
 * the RPC stub instead; non-GET calls opt into single-flight automatically,
 * so post-mutation query data rides back on the response (see query.ts).
 *
